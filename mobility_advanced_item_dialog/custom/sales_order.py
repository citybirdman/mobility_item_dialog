...
def get_item_details(filters=None):
    ...
    if(filters["item_code"] or  filters["brand"]  or filters["txt"]):
        ...
        # elif(filters["brand"]==""):
        #     condition=("and (name like '%{0}%' or item_name like '%{0}%' ".format(filters["item_code"]))
        # else:
...
def get_qnt_on_batch_warehouse(item,warehouse,price_list):
    ...
    for batchs in all_batch:
        ...
        if batch_values:
            qty = batch_values
        batch.append({"production_year":str((frappe.db.get_value("Batch",{"name":batchs["name"],"item":item},"production_year"))),
